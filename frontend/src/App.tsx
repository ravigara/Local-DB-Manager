    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
                ...
                <article className={`database-card status-${database.status}`} key={database.id}>
                  ...
                  {inspection?.id === database.id && <div className="inspection-panel">
                    ...
                    {tableDetails?.id === database.id && <TableDetailsView details={tableDetails.details} exporting={exportingTable === `${database.id}:${tableDetails.details.tableName}`} onExport={() => void exportTable(database, tableDetails.details.tableName)} />}
                     <div className="query-panel"><div className="query-heading"><div><span className="eyebrow">QUERY CONSOLE</span><h4>{database.engine === "mongodb" ? "Run MongoDB operation" : "Run SQL"}</h4></div><span className="sql-badge">{engineLabel(database.engine)}</span></div><textarea value={query} onChange={event => setQuery(event.target.value)} spellCheck={false} placeholder={database.engine === "mongodb" ? '{"collection":"users","operation":"find","filter":{}}' : undefined} /><button className="primary-button small" onClick={() => void runQuery(database)} disabled={queryLoadingId !== null || actionsDisabled}><Icon name="play" />{queryLoadingId === database.id ? "Running..." : "Run query"}</button>{queryResult?.id === database.id && <QueryResultView result={queryResult.result} />}</div>
                  </div>}
                </article>
              );
            })}
          ...
          <section className="create-prompt"><div><span className="eyebrow">LOCAL WORKSPACE</span><h2>Need another environment?</h2><p>Provision a persistent MySQL, PostgreSQL, MariaDB, or MongoDB container with a few details.</p></div><button className="secondary-button" type="button" onClick={() => { setError(""); setCreateOpen(true); }}><Icon name="plus" /> Create environment</button></section>
      ...
      {createOpen && <div className="modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget && !loading) setCreateOpen(false); }}>
        <section className="create-dialog" role="dialog" aria-modal="true" aria-labelledby="create-dialog-title" onMouseDown={event => event.stopPropagation()}>
           ...
           <form className="create-form" onSubmit={event => { event.preventDefault(); void createDatabase(); }} noValidate>
             <div className="form-field"><label htmlFor="database-engine">Database engine</label><select id="database-engine" value={engine} onChange={event => { const nextEngine = event.target.value as DatabaseEngine; setEngine(nextEngine); setPort(defaultPortForEngine(nextEngine)); setQuery(nextEngine === "mongodb" ? '{"collection":"users","operation":"find","filter":{}}' : "SELECT 1 AS ok;"); }}><option value="mysql">MySQL 8.4</option><option value="postgresql">PostgreSQL 17</option><option value="mariadb">MariaDB 11.4</option><option value="mongodb">MongoDB 8.0</option></select></div>
            ...
            <div className="form-field"><label htmlFor="database-name">Database name</label><input id="database-name" data-testid="database-name" type="text" placeholder="payments_dev" value={databaseName} autoComplete="off" onChange={event => setDatabaseName(event.target.value)} /></div>
             ...
             <p className="field-help"><Icon name="database" size={13} /> Data persists in a named Docker volume. PostgreSQL uses the public schema, MySQL and MariaDB use their default schema, and MongoDB uses collections.</p>
