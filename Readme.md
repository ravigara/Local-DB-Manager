...
This repository is an **active MVP / early-stage implementation**. The current end-to-end path is implemented around MySQL containers managed by Docker Desktop.

Current implementation status:

- Electron desktop shell with a secure preload/API boundary (`contextIsolation: true`, `nodeIntegration: false`).
- MySQL 8.4 container creation with a named persistent Docker volume and health-check readiness polling.
- Persistent instance metadata and encrypted credentials in Electron's application data directory.
- Start, stop, restart, status, logs, container removal, and permanent deletion.
- Starting an instance after its container was removed recreates the container against the retained volume; the volume is never removed by the container-only action.
- MySQL ping, database/table listing, table metadata and sample rows, parameter-capable SQL execution, CSV export, and SQL backup/restore.
- Validation for names, database identifiers, ports, duplicate environments, and missing Docker Desktop.

Docker Desktop must be running for lifecycle, connection, backup, and restore operations. `npm.cmd start` builds the frontend, then starts Electron against that built frontend. The Vite build uses relative asset paths so the Electron `file://` loader can display the page correctly. Electron GPU acceleration is disabled, with in-process GPU fallback enabled, for compatibility with restricted Windows graphics/remote-desktop environments.

The project is being developed incrementally, so **do not assume that every item described in the target architecture already exists**. When modifying the project, inspect the actual repository first and treat the existing code as the source of truth.
...
node -e "const { isPortAvailable } = require('./dist-electron/utils/PortChecker.js'); Promise.all([isPortAvailable(43123), isPortAvailable(80)]).then(values => { console.log(JSON.stringify(values)); })"
```

The build, lint, Electron runtime check, Docker availability check, and port smoke test passed in development. Docker Desktop was unavailable in that environment, so the manual lifecycle checklist below remains required.

### Manual acceptance checklist

With Docker Desktop running, launch the app with `npm.cmd start` and verify:

1. Create an instance using a test name, database, password, and unused port. It should finish as **Running**.
2. Stop, start, and restart it. Each action should complete only after MySQL is ready.
3. Use **Connect & Inspect**, run `SELECT 1 AS ok;`, inspect a table, and export CSV.
4. Create a backup, create test data, restore the backup, and verify the data.
5. Use **Remove Container**, then **Start**. The container should be recreated and the original data should still be present.
6. Use **Delete Permanently** only on the test instance and confirm the card and Docker volume are gone.
7. Stop Docker Desktop and reopen/refresh the app. Existing instances should show an error state with a useful action error rather than appearing running.
